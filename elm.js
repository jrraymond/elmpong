...
Elm.Vec = Elm.Vec || {};
Elm.Vec.make = function (_elm) {
   "use strict";
   _elm.Vec = _elm.Vec || {};
   if (_elm.Vec.values) return _elm.Vec.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var rotateV = F2(function (angle,v) {    var cs = $Basics.cos(angle);var sn = $Basics.sin(angle);return {x: cs * v.x - sn * v.y,y: sn * v.x + cs * v.y};});
   var sub = F2(function (u,v) {    return {x: u.x - v.x,y: u.y - v.y};});
   var add = F2(function (u,v) {    return {x: u.x + v.x,y: u.y + v.y};});
   var scale = F2(function (f,v) {    return {x: v.x * f,y: v.y * f};});
   var dot = F2(function (u,v) {    return u.x * v.x + u.y * v.y;});
   var magnitude = function (v) {    return $Basics.sqrt(v.x * v.x + v.y + v.y);};
   var angle = F2(function (u,v) {    return $Basics.acos(A2(dot,u,v) / (magnitude(u) * magnitude(v)));});
   var alter = F2(function (f,v) {    return {x: f(v.x),y: f(v.y)};});
   var normalize = function (v) {    var m = $Basics.sqrt(Math.pow(v.x,2) + Math.pow(v.y,2));return _U.eq(m,0) ? {x: 0,y: 0} : {x: v.x / m,y: v.y / m};};
   var jV = {x: 0,y: 1};
   var iV = {x: 1,y: 0};
   var unitV = normalize({x: 1,y: 1});
   var zeroV = {x: 0,y: 0};
   var Vec = F2(function (a,b) {    return {x: a,y: b};});
   return _elm.Vec.values = {_op: _op
                            ,Vec: Vec
                            ,zeroV: zeroV
                            ,unitV: unitV
                            ,iV: iV
...
Elm.Main.make = function (_elm) {
   ...
   $Vec = Elm.Vec.make(_elm),
   $Window = Elm.Window.make(_elm);
   var _op = {};
   var infoMB = $Signal.mailbox(false);
   var infoBtn = A2($Graphics$Input.button,A2($Signal.message,infoMB.address,true),"Info");
   var irange = F3(function (lo,hi,step) {    return _U.cmp(lo,hi) > -1 ? _U.list([]) : A2($List._op["::"],lo,A3(irange,lo + step,hi,step));});
   var frange = F3(function (lo,hi,step) {    return _U.cmp(lo,hi) > -1 ? _U.list([]) : A2($List._op["::"],lo,A3(frange,lo + step,hi,step));});
   var getCtrl = F2(function (dirsig,firesig) {
      return A3($Signal.map2,
      F2(function (dirs,fire) {    return {rotation: $Basics.toFloat($Basics.negate(dirs.x)),thrust: $Basics.toFloat($Basics.abs(dirs.y)),fire: fire};}),
      dirsig,
      firesig);
   });
   ...
   delta,
   A6($Signal.map5,
   F5(function (r,p,dt,p0,p1) {    return {reset: r,pause: p,player0: p0,player1: p1,dt: dt};}),
   $Keyboard.isDown($Char.toCode(_U.chr("R"))),
   $Keyboard.isDown($Char.toCode(_U.chr("P"))),
   delta,
   A2(getCtrl,$Keyboard.arrows,$Keyboard.ctrl),
   A2(getCtrl,$Keyboard.wasd,$Keyboard.shift)));
   var planetC = A3($Color.rgb,80,80,20);
   var whiteC = A3($Color.rgb,250,250,250);
   var spaceC = A3($Color.rgb,0,0,0);
   var toTxt = function (_p0) {    return $Graphics$Element.leftAligned($Text.fromString(_p0));};
   var info = $Text.fromString("up[w] to thrust, left/right[a, d] to rotate, ctrl[shift] to fire, p to pause, r to restart");
   var viewScore = F2(function (p0,p1) {
      return $Graphics$Collage.toForm($Graphics$Element.centered(A2($Text.height,
      20,
      $Text.monospace(A2($Text.color,
      whiteC,
      $Text.fromString(A2($Basics._op["++"],
      "Player 0: ",
      A2($Basics._op["++"],$Basics.toString(p0.life),A2($Basics._op["++"],"\nPlayer 1: ",$Basics.toString(p1.life))))))))));
   });
   var viewBullet = F2(function (c,b) {
      return A2($Graphics$Collage.move,
      {ctor: "_Tuple2",_0: b.pos.x,_1: b.pos.y},
      A2($Graphics$Collage.outlined,$Graphics$Collage.solid(c),$Graphics$Collage.circle(b.radius)));
   });
   var playerInfo = function (p) {
      return A2($Basics._op["++"],
      "{px: ",
      A2($Basics._op["++"],
      $Basics.toString($Basics.round(p.body.pos.x)),
      A2($Basics._op["++"],
      ", py: ",
      A2($Basics._op["++"],
      $Basics.toString($Basics.round(p.body.pos.y)),
      A2($Basics._op["++"],
      "} ",
      A2($Basics._op["++"],
      "{vx: ",
      A2($Basics._op["++"],
      $Basics.toString($Basics.round(p.body.vel.x)),
      A2($Basics._op["++"],
      ", vy: ",
      A2($Basics._op["++"],
      $Basics.toString($Basics.round(p.body.vel.y)),
      A2($Basics._op["++"],
      "} ",
      A2($Basics._op["++"],
      "{ax: ",
      A2($Basics._op["++"],
      $Basics.toString($Basics.round(p.body.acc.x)),
      A2($Basics._op["++"],
      ", ay: ",
      A2($Basics._op["++"],
      $Basics.toString($Basics.round(p.body.acc.y)),
   ...
   var gravity = F2(function (m0,m1) {
      ...
      var k = 6.674 * Math.pow(10,-11);
      var d2 = Math.pow(m1.pos.x - m0.pos.x,2) + Math.pow(m1.pos.y - m0.pos.y,2);
      var f = k * m1.mass * m0.mass / d2;
      return A2($Vec.scale,f,dir);
   });
   var updateBody = F3(function (dt,forces,b) {
      var acc = A2($Vec.scale,1 / b.mass,A3($List.foldr,$Vec.add,{x: 0,y: 0},forces));
      var v$ = A2($Vec.add,acc,b.vel);
      var p$ = A2($Vec.add,A2($Vec.scale,dt,v$),b.pos);
   ...
   var calcHits = function () {
      var go = F3(function (accum,p,bs) {
         go: while (true) {
            var _p1 = bs;
            if (_p1.ctor === "[]") {
                  return {ctor: "_Tuple2",_0: accum,_1: p};
               } else {
                  var _p3 = _p1._1;
                  var _p2 = _p1._0;
                  if (A2(intersects,_p2,p.body)) {
                        var _v1 = accum,_v2 = _U.update(p,{life: p.life - $Basics.round(_p2.mass)}),_v3 = _p3;
                        accum = _v1;
                        p = _v2;
                        bs = _v3;
                        continue go;
                     } else {
                        var _v4 = A2($List._op["::"],_p2,accum),_v5 = p,_v6 = _p3;
                        accum = _v4;
                        p = _v5;
                        bs = _v6;
                        continue go;
   ...
   var collision = F4(function (elasticity,bodys,old,nu) {
      collision: while (true) {
         ...
         if (_p4.ctor === "[]") {
               ...
               var _p5 = _p4._0;
               if (A2(intersects,_p5,nu)) {
                     var n = $Vec.normalize(A2($Vec.sub,_p5.pos,old.pos));
                     var u = A2($Vec.scale,A2($Vec.dot,nu.vel,n),n);
                     var w = A2($Vec.sub,nu.vel,u);
                     var v$ = A2($Vec.scale,elasticity,A2($Vec.sub,w,u));
                     return _U.update(old,{vel: v$});
                  } else {
                     var _v8 = elasticity,_v9 = _p4._1,_v10 = old,_v11 = nu;
                     elasticity = _v8;
                     bodys = _v9;
                     old = _v10;
                     nu = _v11;
   ...
   var bounds = F4(function (elasticity,box,old,nu) {
      return _U.cmp(nu.pos.x,box.x) < 0 || _U.cmp(nu.pos.x,box.x + box.w) > 0 ? _U.update(nu,
      {pos: old.pos,vel: A2($Vec.scale,elasticity,{x: $Basics.negate(nu.vel.x),y: nu.vel.y})}) : _U.cmp(nu.pos.y,box.y) < 0 || _U.cmp(nu.pos.y,
      box.y + box.h) > 0 ? _U.update(nu,{pos: old.pos,vel: A2($Vec.scale,elasticity,{x: nu.vel.x,y: $Basics.negate(nu.vel.y)})}) : nu;
   });
   var updateBullet = F5(function (elasticity,boundary,bodys,dt,bullet) {
      var gravs = A2($List.map,gravity(bullet),bodys);
      return A4(bounds,elasticity,boundary,bullet,A4(collision,elasticity,bodys,bullet,A3(updateBody,dt,gravs,bullet)));
   });
   var updatePlayer = F8(function (c,boundary,bodys,dt,rot,thrust,fire,p) {
      var bs$ = fire && _U.cmp(p.bullets,0) > 0 ? p.bullets - 1 : p.bullets;
      var gravs = A2($List.map,function (b) {    return A2(gravity,p.body,b);},bodys);
      var tmp = rot * c.rotationSensitivity + p.dir;
      var dir$ = _U.cmp(tmp,2 * $Basics.pi) > 0 ? tmp - 2 * $Basics.pi : _U.cmp(tmp,$Basics.negate(2) * $Basics.pi) < 0 ? tmp + 2 * $Basics.pi : tmp;
      var engine = A2($Vec.rotateV,dir$,A2($Vec.scale,c.enginePower * thrust,{x: 1,y: 0}));
      var b$ = A3(updateBody,dt,A2($List._op["::"],engine,gravs),p.body);
      var nextbdy = A4(bounds,c.collisionElasticity,boundary,p.body,A4(collision,c.collisionElasticity,bodys,p.body,b$));
      return _U.update(p,{body: nextbdy,dir: dir$,thrust: !_U.eq(thrust,0),bullets: bs$});
   });
   var defaultConfig = {rotationSensitivity: 0.1,enginePower: 10,bulletRadius: 10,bulletMass: 1,maxBullets: 100,collisionElasticity: 0.5};
   var epMB = $Signal.mailbox(defaultConfig.enginePower);
   var epInp = A2($Graphics$Input.dropDown,
   $Signal.message(epMB.address),
   A2($List.map,function (x) {    return {ctor: "_Tuple2",_0: $Basics.toString($Basics.round(x)),_1: x};},A3(frange,0,51,1)));
   var rsMB = $Signal.mailbox(defaultConfig.rotationSensitivity);
   var rsInp = A2($Graphics$Input.dropDown,
   $Signal.message(rsMB.address),
   A2($List.map,function (x) {    return {ctor: "_Tuple2",_0: $Basics.toString(x),_1: x};},A3(frange,5.0e-2,0.51,5.0e-2)));
   var brMB = $Signal.mailbox(defaultConfig.bulletRadius);
   var brInp = A2($Graphics$Input.dropDown,
   $Signal.message(brMB.address),
   A2($List.map,function (x) {    return {ctor: "_Tuple2",_0: $Basics.toString(x),_1: x};},A3(frange,1,50,1)));
   var mbMB = $Signal.mailbox(defaultConfig.maxBullets);
   var mbInp = A2($Graphics$Input.dropDown,
   $Signal.message(mbMB.address),
   A2($List.map,function (x) {    return {ctor: "_Tuple2",_0: $Basics.toString(x),_1: x};},A2($List._op["::"],1,A3(irange,5,101,5))));
   var elasticityMB = $Signal.mailbox(defaultConfig.collisionElasticity);
   var elasticityInp = A2($Graphics$Input.dropDown,
   $Signal.message(elasticityMB.address),
   A2($List.map,function (x) {    return {ctor: "_Tuple2",_0: $Basics.toString(x),_1: x};},A3(frange,0,1.1,0.1)));
   var viewConfig = A2($Graphics$Element.flow,
   $Graphics$Element.right,
   _U.list([toTxt("Engine Power")
           ,epInp
           ...
           ,elasticityInp]));
   var configSig = A6($Signal.map5,
   F5(function (ep,rs,br,mb,el) {    return {rotationSensitivity: rs,enginePower: ep,bulletRadius: br,bulletMass: 1,maxBullets: mb,collisionElasticity: el};}),
   epMB.signal,
   rsMB.signal,
   brMB.signal,
   mbMB.signal,
   elasticityMB.signal);
   var defaultControl = {rotation: 0,thrust: 0,fire: false};
   var shiftPlayer = F3(function (x,y,p) {
      var b = p.body;
      var b$ = _U.update(b,{pos: {x: p.body.pos.x + x,y: p.body.pos.y + y}});
      return _U.update(p,{body: b$});
   });
   var defaultBody = {pos: $Vec.zeroV,vel: $Vec.zeroV,acc: $Vec.zeroV,mass: 1,radius: 1};
   var defaultPlayer = {body: _U.update(defaultBody,{radius: 10}),dir: 0,thrust: false,life: 0,bullets: defaultConfig.maxBullets};
   var defaultGame = {paused: true
                     ,boundary: {x: -200,y: -200,w: 400,h: 400}
                     ,player0: A3(shiftPlayer,0,100,defaultPlayer)
                     ,player1: A3(shiftPlayer,0,-100,defaultPlayer)
                     ,planets: _U.list([{body: {pos: {x: 200,y: 0},vel: $Vec.zeroV,acc: $Vec.zeroV,mass: 1 * Math.pow(10,14),radius: 25}}
                                       ,{body: {pos: {x: -200,y: 0},vel: $Vec.zeroV,acc: $Vec.zeroV,mass: 5 * Math.pow(10,14),radius: 50}}])
                     ,bullets0: _U.list([])
                     ,bullets1: _U.list([])};
   var Game = F7(function (a,b,c,d,e,f,g) {    return {paused: a,boundary: b,player0: c,player1: d,planets: e,bullets0: f,bullets1: g};});
   var Config = F6(function (a,b,c,d,e,f) {
      return {rotationSensitivity: a,enginePower: b,bulletRadius: c,bulletMass: d,maxBullets: e,collisionElasticity: f};
   });
   var Input = F5(function (a,b,c,d,e) {    return {reset: a,pause: b,player0: c,player1: d,dt: e};});
   var Controls = F3(function (a,b,c) {    return {rotation: a,thrust: b,fire: c};});
   var Planet = function (a) {    return {body: a};};
   ...
   var fire = F2(function (c,p) {
      return {pos: p.body.pos,vel: A2($Vec.scale,bulletVel,A2($Vec.rotateV,p.dir,$Vec.iV)),acc: $Vec.zeroV,mass: c.bulletMass,radius: c.bulletRadius};
   });
   var update = F2(function (_p6,game) {
      ...
      var _p11 = _p7._1;
      var _p10 = _p7._2;
      if (_p12.reset) return defaultGame; else if (A2($Basics.xor,_p12.pause,game.paused)) return _U.update(game,{paused: true}); else {
               var bullets1$ = _p12.player1.fire && _U.cmp(game.player1.bullets,0) > 0 ? A2($List._op["::"],
               A2(fire,_p10,game.player1),
               game.bullets1) : game.bullets1;
               var bullets0$ = _p12.player0.fire && _U.cmp(game.player0.bullets,0) > 0 ? A2($List._op["::"],
               A2(fire,_p10,game.player0),
               game.bullets0) : game.bullets0;
               var pbs = A2($List.map,function (_) {    return _.body;},game.planets);
               var boundary$ = {x: $Basics.negate(_p13) / 2,y: $Basics.negate(_p11) / 2,w: _p13,h: _p11};
               var p0$ = A8(updatePlayer,_p10,boundary$,pbs,_p12.dt,_p12.player0.rotation,_p12.player0.thrust,_p12.player0.fire,game.player0);
               var p1$ = A8(updatePlayer,_p10,boundary$,pbs,_p12.dt,_p12.player1.rotation,_p12.player1.thrust,_p12.player1.fire,game.player1);
               var _p8 = A2(calcHits,p0$,A2($List.map,A4(updateBullet,_p10.collisionElasticity,boundary$,pbs,_p12.dt),bullets1$));
               var uBs1$ = _p8._0;
               var up0$ = _p8._1;
               var _p9 = A2(calcHits,p1$,A2($List.map,A4(updateBullet,_p10.collisionElasticity,boundary$,pbs,_p12.dt),bullets0$));
               var uBs0$ = _p9._0;
               var up1$ = _p9._1;
               return _U.update(game,{paused: false,boundary: boundary$,player0: up0$,player1: up1$,bullets0: uBs0$,bullets1: uBs1$});
            }
   });
   ...
   defaultGame,
   A4($Signal.map3,
   F3(function (_p14,c,i) {    var _p15 = _p14;return {ctor: "_Tuple4",_0: $Basics.toFloat(_p15._0),_1: $Basics.toFloat(_p15._1),_2: c,_3: i};}),
   $Window.dimensions,
   configSig,
   input));
   ...
   var viewGame = F3(function (w,h,g) {
      ...
      h,
      A2($Basics._op["++"],
      _U.list([A2($Graphics$Collage.filled,spaceC,A2($Graphics$Collage.rect,$Basics.toFloat(w),$Basics.toFloat(h)))
              ,A2(viewPlayer,g.player0,p0Color)
              ,A2(viewPlayer,g.player1,p1Color)
              ,A2($Graphics$Collage.move,{ctor: "_Tuple2",_0: $Basics.toFloat(w) / -2.2,_1: $Basics.toFloat(h) / 2.1},A2(viewScore,g.player0,g.player1))
              ,A2($Graphics$Collage.move,
              {ctor: "_Tuple2",_0: $Basics.toFloat(w) / -2.5,_1: 50 - $Basics.toFloat(h) / 2},
              $Graphics$Collage.toForm($Graphics$Element.centered(A2($Text.height,
              12,
              $Text.monospace(A2($Text.color,
              whiteC,
              $Text.fromString(A2($Basics._op["++"],playerInfo(g.player0),A2($Basics._op["++"],"\n",playerInfo(g.player1))))))))))
              ,A2($Graphics$Collage.move,
              {ctor: "_Tuple2",_0: $Basics.toFloat(w) / -3.4,_1: 100 - $Basics.toFloat(h) / 2},
              $Graphics$Collage.toForm($Graphics$Element.centered(A2($Text.height,12,$Text.monospace(A2($Text.color,whiteC,info))))))]),
      A2($Basics._op["++"],
      A2($List.map,
      function (p) {
         return A2($Graphics$Collage.move,
         {ctor: "_Tuple2",_0: p.body.pos.x,_1: p.body.pos.y},
         A2($Graphics$Collage.filled,planetC,$Graphics$Collage.circle(p.body.radius)));
      },
      g.planets),
      A2($Basics._op["++"],A2($List.map,viewBullet(p0Color),g.bullets0),A2($List.map,viewBullet(p1Color),g.bullets1))))));
   });
   var view = F2(function (_p16,g) {
      var _p17 = _p16;
      return A2($Graphics$Element.flow,$Graphics$Element.down,_U.list([viewConfig,A3(viewGame,_p17._0,_p17._1,g)]));
   });
   var main = A3($Signal.map2,view,$Window.dimensions,gameSig);
   return _elm.Main.values = {_op: _op
                             ...
                             ,infoMB: infoMB
                             ,infoBtn: infoBtn
                             ,elasticityMB: elasticityMB
                             ,elasticityInp: elasticityInp
                             ,configSig: configSig
                             ,main: main};
};
