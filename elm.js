...
Elm.Main.make = function (_elm) {
   ...
   var collision = F3(function (bodys,old,nu) {
      collision: while (true) {
         ...
         if (_p4.ctor === "[]") {
               ...
               if (A2(intersects,_p5,nu)) {
                     ...
                     var v$ = A2($Vec.sub,w,u);
                     return _U.update(old,{vel: v$});
                  } else {
                     var _v8 = _p4._1,_v9 = old,_v10 = nu;
                     bodys = _v8;
                     old = _v9;
                     nu = _v10;
                     continue collision;
   ...
   var bounds = F3(function (box,old,nu) {
      return _U.cmp(nu.pos.x,box.x) < 0 || _U.cmp(nu.pos.x,box.x + box.w) > 0 ? _U.update(nu,
      {pos: old.pos,vel: {x: $Basics.negate(nu.vel.x),y: nu.vel.y}}) : _U.cmp(nu.pos.y,box.y) < 0 || _U.cmp(nu.pos.y,box.y + box.h) > 0 ? _U.update(nu,
      {pos: old.pos,vel: {x: nu.vel.x,y: $Basics.negate(nu.vel.y)}}) : nu;
   });
   var updateBullet = F4(function (boundary,bodys,dt,bullet) {
      var gravs = A2($List.map,gravity(bullet),bodys);
      return A3(bounds,boundary,bullet,A3(updateBody,dt,gravs,bullet));
   });
   var updatePlayer = F8(function (c,boundary,bodys,dt,rot,thrust,fire,p) {
      ...
      var nextbdy = A3(bounds,boundary,p.body,A3(collision,bodys,p.body,b$));
      return _U.update(p,{body: nextbdy,dir: dir$,thrust: !_U.eq(thrust,0),bullets: bs$});
   });
   var defaultConfig = {rotationSensitivity: 0.1,enginePower: 1,bulletRadius: 1,bulletMass: 1,maxBullets: 100};
   ...
   A2($List.map,function (x) {    return {ctor: "_Tuple2",_0: $Basics.toString(x),_1: x};},A3(frange,0,1.01,5.0e-2)));
   ...
   _U.list([toTxt("Engine Power"),epInp,toTxt("Rotation Sensitivity"),rsInp,toTxt("Bullet Radius"),brInp,toTxt("Max Bullets"),mbInp]));
   var configSig = A5($Signal.map4,
   F4(function (ep,rs,br,mb) {    return {rotationSensitivity: rs,enginePower: ep,bulletRadius: br,bulletMass: 1,maxBullets: mb};}),
   epMB.signal,
   rsMB.signal,
   brMB.signal,
   mbMB.signal);
   ...
   var update = F2(function (_p6,game) {
      ...
      if (_p12.reset) return defaultGame; else if (A2($Basics.xor,_p12.pause,game.paused)) return _U.update(game,{paused: true}); else {
               ...
               var _p8 = A2(calcHits,p0$,A2($List.map,A3(updateBullet,boundary$,pbs,_p12.dt),bullets1$));
               var uBs1$ = _p8._0;
               var up0$ = _p8._1;
               var _p9 = A2(calcHits,p1$,A2($List.map,A3(updateBullet,boundary$,pbs,_p12.dt),bullets0$));
               var uBs0$ = _p9._0;
               var up1$ = _p9._1;
   ...
   var Config = F5(function (a,b,c,d,e) {    return {rotationSensitivity: a,enginePower: b,bulletRadius: c,bulletMass: d,maxBullets: e};});
